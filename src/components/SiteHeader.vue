...
function isActive(item: NavItem): boolean {
  if (item.path && (props.currentPath === item.path || props.currentPath.startsWith(item.path + '/'))) return true
  if (item.children) return item.children.some(c => c.path && (props.currentPath === c.path || props.currentPath.startsWith(c.path + '/')))
  return false
}
  ...
  >
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        <!-- Logo -->
        <a href="/" class="flex items-center gap-2.5 shrink-0 group" @click="closeMobile">
              ...
              >
                ...
                >
                  <a
                    ...
                    :class="[currentPath === child.path ? 'text-elf-blue dark:text-elf-blue' : '']"
                  >
                    {{ child.title }}
                  </a>
                </div>
